...
const User = () => {
        ...
        const fetchUsers = async () => {
            try {
                const { data } = await axios.get(
                    "http://localhost:4000/api/user/user/info",
    ...
    const handleDeleteUser = async (Id) => {
        try {
            await axios.delete(`http://localhost:4000/api/user/user/delete/${Id}`, { withCredentials: true });
    ...
    return (
        <section className="page">
            <div className="flex flex-col justify-center items-center head_text text-center mb-8">
                <h1 className="red_gradient text-4xl font-bold mb-2">User</h1>
            </div>
            <div className="flex items-center justify-between mt-8">
                ...
                <Link style={{textDecoration:'none'}} to='/create_user' className='btn btn-primary bg-green-700 hover:bg-green-800 font-medium rounded-lg text-sm px-4 py-2 ml-1 text-white'>
                    Add User
                </Link>
            </div>
                        ...
                        {users && users.length > 0 ? (
                            users.map((manager, index) => (
                                <tr key={index}>
                                    ...
                                    <td className="border px-6 py-4 whitespace-nowrap text-sm text-gray-700">{manager.email}</td>
                                ...
                                />
                                    ...
                                    <td className="border px-6 py-4 whitespace-nowrap text-sm text-gray-700">{manager.dob.substring(0, 10)}</td>
                                    <td className="border px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                        <button onClick={() => handleDeleteUser(manager._id)} className="text-red-600 hover:text-red-800">
                                            Delete
                                        </button>
                                    </td>
                                </tr>
